...
AGENTS = {
    "universal": ("🤖 Универсальный", "Для любых вопросов"),
    "confectioner": ("🧁 Кондитер", "Рецепты и технологии"),
    "designer": ("🎨 Дизайнер", "Дизайн тортов и промпты"),
    "marketing": ("📈 Маркетолог", "Продажи и продвижение"),
    "technical": ("🛠 Технический", "Код, API и оборудование"),
}

def agent_keyboard(current: str = "universal") -> InlineKeyboardMarkup:
    rows=[]
...
async def send_agent_menu(target, user_id: int):
    current=await db.get_user_agent(user_id)
    title,desc=AGENTS.get(current,AGENTS["universal"])
    await target.answer(
        f"<b>Выберите AI-агента</b>\n\nСейчас выбран: <b>{title}</b>\n{desc}",
...
async def start(m:Message,state:FSMContext):
    await db.upsert_user(m.from_user); await state.clear(); await m.answer("🧁 <b>CakeHub AI Bot 3.3 Final</b>\n\nПросто напишите сообщение или отправьте фотографию.\n\nКоманды: /agent — выбрать AI-агента, /image — создать изображение, /gallery — открыть галерею.\n\nВ группе позовите меня: «КейкХаб, помоги». После вызова я буду отвечать постоянно. Отключить: /bot_off", reply_markup=ReplyKeyboardRemove()); current=await db.get_user_agent(m.from_user.id); await m.answer("🤖 Выберите специализацию помощника:", reply_markup=agent_keyboard(current))
...
async def agent_cmd(m: Message):
    await send_agent_menu(m, m.from_user.id)

@router.callback_query(F.data.startswith("agent:"))
async def choose_agent(c: CallbackQuery):
    agent=c.data.split(":",1)[1]
    if agent not in AGENTS:
        return await c.answer("Неизвестный агент", show_alert=True)
    await db.set_user_agent(c.from_user.id, agent)
    title,desc=AGENTS[agent]
    await c.message.edit_text(
        f"✅ Выбран агент: <b>{title}</b>\n{desc}\n\nТеперь просто напишите вопрос.",
        reply_markup=agent_keyboard(agent),
    )
    await c.answer("Агент выбран")
...
async def got_photo(m:Message,state:FSMContext,bot:Bot):
    ...
    if mode=="analyze":
        agent=await db.get_user_agent(m.from_user.id); txt,model=await gh.analyze_image(str(path),"Проанализируй дизайн, композицию, текст, цвета и предложи конкретные улучшения.",agent); await m.answer(f"🔍 <b>Анализ</b>\n{txt}\n\n<i>{model}</i>"); await state.clear()
...
async def chat(m: Message, bot: Bot):
    ...
    try:
        agent = await db.get_user_agent(m.from_user.id)
        text, model = await gh.chat(hist[:-1], prompt, agent)
