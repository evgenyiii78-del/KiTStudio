...
async def stats_cmd(m:Message):
    ...
    u,msg,g=await db.stats(); await m.answer(f"📊 Пользователей: {u}\nСообщений: {msg}\nГенераций: {g}")

@router.message(Command("users"))
async def users_cmd(m:Message):
    if m.from_user.id not in settings.admins:return
    rows=await db.recent_users(50)
    text="\n".join(f"{uid} | @{username or '-'} | {first or '-'} | {seen[:19]}" for uid,username,first,seen in rows) or "Пользователей нет"
    await m.answer("👥 <b>Последние пользователи</b>\n<pre>"+text[:3800]+"</pre>")

@router.message(Command("broadcast"))
async def broadcast_cmd(m:Message,bot:Bot):
    if m.from_user.id not in settings.admins:return
    text=m.text.partition(" ")[2].strip()
    if not text:return await m.answer("Использование: /broadcast текст сообщения")
    ok=bad=0
    for uid in await db.all_user_ids():
        try: await bot.send_message(uid,text); ok+=1
        except Exception: bad+=1
    await m.answer(f"📣 Рассылка завершена. Доставлено: {ok}, ошибок: {bad}")
