...
async def connect_telegram() -> tuple[Bot, object, str]:
    ...
    for source, token in telegram_token_candidates():
        ...
        try:
            ...
            last_error = exc
            logging.warning("Telegram token from %s rejected: %s", source, type(exc).__name__)
            await bot.session.close()

    raise RuntimeError(
        "Telegram не принял ни BOT_TOKEN, ни TELEGRAM_BOT_TOKEN. Проверь токен в Bothost."
    ) from last_error
...
async def main() -> None:
    ...
    try:
        ...
        logging.info("KiTstudio AI Bot 3.5.0 started as @%s", me.username)
        logging.info("Telegram token source: %s", token_source)
        logging.info("AITunnel API key: loaded")
        logging.info("Mode: image generation")
        logging.info("Image model EFFECTIVE: gpt-image-2")
